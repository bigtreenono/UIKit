...
- (void)setPopupViewControllerCallBack:(popupViewControllerCallBack)popupViewControllerCallBack
{
    objc_setAssociatedObject(self, @selector(popupViewControllerCallBack), popupViewControllerCallBack, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (popupViewControllerCallBack)popupViewControllerCallBack
{
    return objc_getAssociatedObject(self, @selector(popupViewControllerCallBack));
}
...
+ (instancetype)showPopupInViewController:(UIViewController *)viewController style:(STPopupStyle)style callBack:(popupViewControllerCallBack)popupViewControllerCallBack
{
    ...
    [vc showPopupInViewController:viewController style:style callBack:popupViewControllerCallBack];
    return vc;
}
...
{
    self.popupViewControllerCallBack = popupViewControllerCallBack;
    STPopupController *popupController = [[STPopupController alloc] initWithRootViewController:self];
    popupController.style = style;
    popupController.cornerRadius = style == STPopupStyleFormSheet ? 5 : 0;
    popupController.enableDismissWhenTappedBackground = style == STPopupStyleBottomSheet;
